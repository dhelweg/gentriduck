...
--   The Aufwertung OUTCOME is the DECLINING einfach share over vintages (built separately).
...
--   Vulnerability-positive: pct_einfach is positive (high share = more headroom/vulnerable),
...
--   requires sign-FLIP if pooled into vulnerability composite (high score = low vulnerability).
--
-- =============================================================================
-- Methodology
-- =============================================================================
...
--   A PLR-vintage has wohnlage_low_n = TRUE when SUM(n_addresses) for that PLR-vintage < 10.
...
--
-- Materialization: table (not view) — window function in the final SELECT causes a DuckDB
...
with
    ...
    -- as 25833); guard is applied as a WHERE to reject such rows rather than fail silently.
    wohnlage as (
        select
            vintage,
            city_code,
        ...
        where geometry_wkb is not null
            ...
            -- ST_Centroid returns a Point regardless of geometry type, so ST_Y is safe here.
            and st_y(st_centroid(st_geomfromwkb(geometry_wkb))) between 5.79e6 and 5.84e6
    ),
...
select
    ...
    -- wohnlage_low_n flag (geo condition 7): TRUE when PLR-vintage has < 10 total addresses.
    -- Downstream consumers must NULL derived scalars (wohnlage_score, est_rent) for these PLRs.
    t.total_n_addresses < 10 as wohnlage_low_n
from aggregated as a
inner join plr_vintage_total as t
