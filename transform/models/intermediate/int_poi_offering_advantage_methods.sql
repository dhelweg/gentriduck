...
-- alongside the existing faithful nested-LQ. The four PROMOTED modes named in
-- the maintainer's maximal-breadth knob (z-score/binomial-SLQ, Getis-Ord,
-- density, per-capita) are DELIBERATELY DEFERRED to a follow-on ticket: they
-- each need an external join/tool this model does not introduce (EWR
-- population for per-capita, ST_Area geometry for density, a Queen-contiguity
-- spatial-weights matrix for Getis-Ord) and OA-D0 geo sign-off Condition C9
-- explicitly flags Getis-Ord's `esda`/W-matrix promotion as possibly straining
-- the "no new tool, pure DuckDB" ADR-0024 claim -- "route to the
-- system-architect to confirm before D3/D6". Rather than block this whole
-- ticket on that architect ruling, this slice ships the six methods that are
-- pure functions of int_poi_offering_advantage's ALREADY-COMPUTED,
-- ALREADY-TESTED stock columns (no new join, no new tool, no new data
-- source) -- see #240 tracker comment for the split rationale.
--
-- =============================================================================
...
-- =============================================================================
--
-- Method definitions (six methods x three taxonomy levels = 18 value
...
--
-- Deferred (NOT built here -- see header): z-score/binomial-SLQ, Getis-Ord
-- Gi*, density, per-capita (OA-D3b, pending system-architect ruling on the
-- Getis-Ord/esda tooling boundary per OA-D0 geo sign-off C9).
--
