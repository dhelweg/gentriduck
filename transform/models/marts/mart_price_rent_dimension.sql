...
-- =============================================================================
-- Smith (1979) rent gap: BRW LEVEL = capitalised ground-rent level (price-surface context,
--   one term of the gap); rent-gap REALISATION = BRW CHANGE (brw_trend, built separately).
...
--   ortsübliche Vergleichsmiete estimate — conservative, structural, NOT leading-edge rent.
--   Frame toward displacement-protection use, not investment-opportunity ranking (domain D12).
...
-- OECD/JRC (2008): winsorized z-score + rank/percentile per city × vintage; common polarity
--   before aggregation (but levels here are structural — NOT blended into Status×Dynamik).
...
-- ADR-0003 §Price/rent (P-A Bodenrichtwerte, P-B Mietspiegel/Wohnlage); ADR-0008 lead-lag.
--
...
--   Headline for G2: brw_rank / brw_percentile (heavy-tailed land values; z-score secondary).
--
-- Ecological fallacy guardrail (domain D10, G-2):
--   All values here are PLR-LEVEL aggregates. NOT individual or building-level statements.
--   BRW is coarser than PLR (1,621 zones, area-interpolated) — do not imply parcel precision.
...
--   Bernt 2016). NOT an investment-opportunity surface. A low land value coinciding with a
...
with
    ...
    wohnlage_wide as (
        select
            ...
            area_code,
            area_vintage,
            sum(case when wohnlage = 'einfach' then pct_wohnlage else 0.0 end)
                as pct_einfach,
            sum(case when wohnlage = 'mittel' then pct_wohnlage else 0.0 end)
                as pct_mittel,
            sum(case when wohnlage = 'gut' then pct_wohnlage else 0.0 end)
                as pct_gut,
    ...
    --   '1950_1964' exists in all Mietspiegel vintages (2017, 2019, 2021, 2023, 2024, 2026)
    --   and is therefore the stable anchor across the full series without schema-drift breaks.
    --   See header note on the 1973+ drift and bucket split in 2024/2026 editions.
    mietspiegel_fixed as (
        select
            vintage as ms_vintage,
            wohnlage,
            rent_low,
            rent_mid,
            rent_high
        ...
        where size_bucket = '60_to_90'
            and year_built_bucket = '1950_1964'
    ),

    -- Pivot Mietspiegel to one row per vintage: three wohnlage columns each for low/mid/high.
    ...
    wohnlage_with_rent as (
        ...
        -- The lateral-style join below takes the max Mietspiegel vintage that is <= Wohnlage.
        inner join mietspiegel_pivot as m
            on m.ms_vintage = (
    ...
    -- BRW × Wohnlage alignment:
    -- BRW has yearly data (2017–2024); Wohnlage has biennial data (2017,2019,2021,2023,2026).
    -- We produce one row per BRW snapshot_year, attaching the nearest-<= Wohnlage vintage.
    ...
    brw_aligned as (
        ...
        from brw as b
        left join wohnlage_with_rent as w
            ...
            and w.snapshot_year = (
                ...
                where wwr2.city_code = b.city_code
    ...
    -- Add Wohnlage-only rows (vintages 2026 have no BRW match since BRW only goes to 2024).
    ...
    -- Normalization (geo condition 13):
    -- Winsorized (1%/99%) z-score per (city_code, snapshot_year) over inhabited residential
    -- PLRs only (exclude NULL/low-n PLRs from the moments so they don't dilute the distribution).
    -- brw_zscore, wohnlage_zscore, est_rent_zscore.
    -- brw_rank, brw_percentile: rank and percent_rank of BRW value (rank is the headline
    ...
    ws_quantiles as (
        ...
        from combined
        where wohnlage_score is not null
            and (wohnlage_low_n is null or wohnlage_low_n = false)
        group by city_code, snapshot_year
    ),
    ...
    er_quantiles as (
        ...
        from combined
        where est_rent_mid is not null
            and (wohnlage_low_n is null or wohnlage_low_n = false)
        group by city_code, snapshot_year
    ),
    ...
    brw_moments as (
        ...
        from combined as c
        inner join brw_quantiles as q
            on c.city_code = q.city_code
            and c.snapshot_year = q.snapshot_year
        where c.brw_weighted_avg_eur_m2 is not null
    ...
    ws_moments as (
        ...
        from combined as c
        inner join ws_quantiles as q
            on c.city_code = q.city_code
            and c.snapshot_year = q.snapshot_year
        where c.wohnlage_score is not null
            and (c.wohnlage_low_n is null or c.wohnlage_low_n = false)
        group by c.city_code, c.snapshot_year
    ),
    ...
    er_moments as (
        ...
        from combined as c
        inner join er_quantiles as q
            on c.city_code = q.city_code
            and c.snapshot_year = q.snapshot_year
        where c.est_rent_mid is not null
            and (c.wohnlage_low_n is null or c.wohnlage_low_n = false)
        group by c.city_code, c.snapshot_year
    )
...
select
    ...
    c.wohnlage_low_n,
    -- wohnlage_score: ordinal mean (einfach=1, mittel=2, gut=3); ORDINAL-MEAN APPROXIMATION.
    ...
    -- Bestandsmiete/ortsübliche Vergleichsmiete (Holm 2010): lagging, conservative proxy.
    ...
    -- over non-NULL, non-low-n PLRs (geo condition 13; OECD/JRC 2008 polarity convention).
    --
    -- brw_zscore polarity: ambiguous (domain D2). High BRW = price-surface/consolidation;
    --   if pooled into vulnerability composite, FLIP sign (high BRW = low vulnerability headroom).
    --   Context covariate, NOT a vulnerability score.
    -- wohnlage_zscore polarity: high score = desirable = consolidated = LOW headroom;
    --   FLIP sign for vulnerability composite. For vulnerability framing, pct_einfach is +positive.
    -- est_rent_zscore polarity: affordability-negative (high = less affordable); Bestandsmiete-lagging.
    (
    ...
    -- brw_rank: ascending (1 = lowest BRW in city-year; higher rank = higher land value).
