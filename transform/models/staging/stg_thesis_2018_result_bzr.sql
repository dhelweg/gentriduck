-- staging: 2018 thesis golden output at BZR (Bezirksregion) level
-- source:   reference/goldens/20180909_result_full_bzr.csv  (ODbL)
-- encoding: latin-1  (raum_desc contains German umlauts)
-- 137 rows; key columns only — full 1722-column CSV not carried forward.
-- area_level = 'bzr', city_code = 'BER', variant = 'standard'
{{
    config(
        materialized="view",
