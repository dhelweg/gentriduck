...
def _write_manifest(out_dir: Path) -> None:
    """ADR-0016: record this source's current on-disk outputs (all years present, not
    just the years this run touched) in the committed manifest."""
    ...
    write_manifest_entry(
        ...
        upstream_vintage=",".join(
            str(y) for y in sorted({int(p.stem) for p in found if p.stem.isdigit()})
        ),
        output_paths=found,
